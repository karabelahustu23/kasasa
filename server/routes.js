...
router.all('/api/index.php', wrap((req, res) => {
  ...
  if (table === 'products') {
    if (method === 'GET') {
      const rid = req.query.restaurant_id; if (!rid) H.errorResponse('restaurant_id gerekli');
      // Elle verilen sira (sort_order) onceliklidir; hic siralanmamissa (hepsi 0)
      // eski davranis (one cikanlar + en yeni) devreye girer.
      const rows = db.prepare('SELECT * FROM products WHERE restaurant_id=? ORDER BY sort_order ASC, is_featured DESC, created_at DESC').all(rid);
      rows.forEach(r => r.translations = parseJsonField(r.translations, {}));
      return j(res, rows);
    }
    if (method === 'POST') {
      ...
      const maxSortP = db.prepare('SELECT COALESCE(MAX(sort_order),0)+1 AS n FROM products WHERE restaurant_id=?').get(rid).n;
      const sortVal = (body.sort_order !== undefined && body.sort_order !== '' && body.sort_order !== null) ? Number(body.sort_order) : maxSortP;
      db.prepare(`INSERT INTO products (id,restaurant_id,category_id,name,description,price,image_url,is_available,is_featured,translations,stock,hidden_from_menu,vat_exempt,sort_order) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
        .run(pid, rid, body.category_id || null, body.name, body.description || null, Number(body.price), body.image_url || null, isAvailable, isFeatured, JSON.stringify(body.translations || {}), stockVal, isHidden, isVatExempt, sortVal);
    ...
    if (method === 'PUT' && id) {
      ...
      for (const f of ['category_id', 'name', 'description', 'price', 'image_url', 'is_available', 'is_featured', 'translations', 'stock', 'hidden_from_menu', 'vat_exempt', 'sort_order']) {
  ...
  if (table === 'orders') {
    if (method === 'POST' && action === 'void_and_reopen') {
      ...
      if (origItems.length) {
        ...
        const insItem = db.prepare('INSERT INTO order_items (id,restaurant_id,order_id,product_id,product_name,variant_name,quantity,price,ingredient_cost,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)');
        ...
        origItems.forEach((item, idx) => {
          ...
          insItem.run(iid, order.restaurant_id, newId, item.product_id, item.product_name, item.variant_name, item.quantity, item.price, itemCosts[idx] ?? null, H.nowSql());
        });
      }
    ...
    if (method === 'POST' && action !== 'close_day') {
      ...
      db.prepare(`INSERT INTO orders (id,restaurant_id,table_id,status,total,discount_amount,note,is_paid,vat_amount,employee_name,created_at) VALUES (?,?,?,?,?,?,?,0,?,?,?)`)
        .run(oid, rid, body.table_id || null, 'pending', finalTotal, discountAmt, body.note || null, vatAmt, body.employee_name || null, H.nowSql());
      let itemCosts = {};
      if (body.order_items && body.order_items.length) {
        itemCosts = H.deductRecipeStock(rid, body.order_items, oid);
        const ins = db.prepare('INSERT INTO order_items (id,restaurant_id,order_id,product_id,product_name,variant_name,quantity,price,ingredient_cost,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)');
        ...
        body.order_items.forEach((item, idx) => {
          ...
          ins.run(item.id, rid, oid, item.product_id || null, item.product_name, item.variant_name || null, item.quantity, item.price, itemCosts[idx] ?? null, H.nowSql());
        });
  ...
  if (table === 'order_refunds') {
    ...
    if (method === 'POST') {
      ...
      if (!body.order_id) H.errorResponse('order_id gerekli');
      const rfid = H.uuid();
      db.prepare('INSERT INTO order_refunds (id,restaurant_id,order_id,amount,reason,employee_name,created_at) VALUES (?,?,?,?,?,?,?)')
        .run(rfid, rid, body.order_id, Number(body.amount || 0), body.reason || null, body.employee_name || null, H.nowSql());
  ...
  if (table === 'order_items') {
    if (method === 'POST') {
      ...
      items.forEach((item, idx) => {
        item.id = item.id || H.uuid();
        ins.run(item.id, rid, item.order_id, item.product_id || null, item.product_name, item.variant_name || null, Number(item.quantity), Number(item.price), itemCosts[idx] ?? null, H.nowSql());
      });
  ...
  if (table === 'expenses') {
    ...
    if (method === 'POST') {
      ...
      const eid = H.uuid();
      db.prepare('INSERT INTO expenses (id,restaurant_id,amount,description,employee_name,created_at) VALUES (?,?,?,?,?,?)').run(eid, rid, amount, body.description || null, body.employee_name || null, H.nowSql());
  ...
  if (table === 'daily_reports') {
    ...
    if (method === 'POST') {
      // Gövdede id geldiyse ONU kullan (H.uuid() ile ezmek, aynı raporun
      // senkronda iki farklı kimlikle dolaşmasına yol açıyordu).
      const did = H.resolveEntityId(body);
