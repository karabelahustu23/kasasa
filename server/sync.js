        ...
        if (!result.ok && item.method !== 'UPLOAD' && item.entity_id) {
          ...
          if (item.method === 'PUT' && (result.status === 404 || result.status === 410)) {
            ...
            if (!repaired) {
              ...
              // Bunun yerine bu isteği düşürüyoruz: local satır zaten doğru
              // durumda ve bir sonraki çekmede kalem içerikle eşleştirilip
              // sunucunun id'si benimseniyor, durum farkı o id ile yeniden
              // kuyruğa alınıyor. Yani hiçbir bilgi kaybolmuyor.
              console.log('[sync] kalem id\'si sunucuda yok — eşleştirmeye bırakıldı:', item.entity_id);
              result = { ok: true, status: 200 };
            } else {
...
async function alreadyOnRemote(serverUrl, item) {
  ...
  // b) Siparişlerde: sunucu kendi id'sini üretmiş olabilir. Aynı masa + aynı
...
function applyOrders(db, orders, opts = {}) {
  ...
  db.transaction((list) => {
    for (const o of list) {
      ...
      const orderChanged = upsertRow(db, 'orders', o);
